 ...
 * Colt Agent: OpenAI API key for scheduled LLM runs.
...
const STORAGE_COLT_AGENT_OPENAI_KEY = 'colt_agent_openai_api_key';
...
const ENV_COLT_AGENT_OPENAI_KEY = import.meta.env.VITE_COLT_AGENT_OPENAI_API_KEY as string | undefined;
...
export function getColtAgentOpenAIKey(): string | undefined {
  const stored = localStorage.getItem(STORAGE_COLT_AGENT_OPENAI_KEY);
  if (stored != null && stored.trim() !== '') return stored.trim();
  return ENV_COLT_AGENT_OPENAI_KEY;
}

export function setColtAgentOpenAIKey(key: string): void {
  const val = key.trim();
  if (val) localStorage.setItem(STORAGE_COLT_AGENT_OPENAI_KEY, val);
  else localStorage.removeItem(STORAGE_COLT_AGENT_OPENAI_KEY);
  window.dispatchEvent(new CustomEvent('colt-road-api-config-saved'));
}
